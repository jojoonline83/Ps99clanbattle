...
function save() {
    try { localStorage.setItem('ps99_cyberpunkleague_v1', JSON.stringify(state)); } catch (_) {}
}
function load() {
    try {
        const raw = localStorage.getItem('ps99_cyberpunkleague_v1');
...
async function loadHistory() {
    const [histRes, namesRes] = await Promise.all([
        fetch(`league_history.json?t=${Date.now()}`, { signal: AbortSignal.timeout(60000) }),
...
function renderLeagueDetail() {
    ...
    document.getElementById('league-detail-sub').textContent = 'Cyberpunk Battle — League';

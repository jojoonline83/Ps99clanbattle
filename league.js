...
function load() {
    try {
        const raw = localStorage.getItem('ps99_luckyblox2_league_v1');
...
async function loadHistory() {
    const [histRes, namesRes] = await Promise.all([
        fetch(`league_history.json?t=${Date.now()}`, { signal: AbortSignal.timeout(30000) }),
...
function renderLeagueDetail() {
    ...
    document.getElementById('league-detail-sub').textContent = 'Lucky Block Part 2 — League';
